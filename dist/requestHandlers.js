...
export const createOpenAPIRequestHandler = (options) => async (req) => {
...
export const createOAuthPasswordFlowHandler = () => async (req) => {
    const formData = await req.formData();
    const collection = req.payload.collections[req.payload.config.admin.user];
    const data = collection.config.auth?.loginWithUsername !== false
        ? {
    ...
    return Response.json({
        ...
        token_type: 'JWT',
        expires_in: response.exp !== undefined
